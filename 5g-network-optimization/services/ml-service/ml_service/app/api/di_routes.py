...
def predict_di():
    ...
    try:
        ...
        if cached_result:
            ...
            services.metrics.track_prediction(
...
async def predict_async_di():
    ...
    try:
        ...
        services.logger.info(
            f"Async prediction completed for UE {req.ue_id}: {result['antenna_id']} "
            f"(confidence: {result['confidence']:.3f})"
        )
        ...
        services.logger.error(f"Async prediction failed for UE {req.ue_id}: {exc}")
...
def train_di():
    ...
    try:
        start = time.time()
        metrics = services.model.train(samples)
        duration = time.time() - start
        
        # Track training metrics
        services.metrics.track_training(
            duration, 
        ...
        services.logger.info(
            f"Training completed in {duration:.2f}s with {metrics.get('samples', 0)} samples"
        )
        ...
        services.logger.error(f"Training failed: {exc}")
        raise ModelError(f"Training failed: {exc}") from exc
    except MemoryError as exc:
        services.logger.error(f"Insufficient memory for training: {exc}")
