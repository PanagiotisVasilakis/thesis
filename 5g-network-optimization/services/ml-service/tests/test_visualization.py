...
def test_coverage_map_endpoint(client, tmp_path):
    ...
    with patch("ml_service.api.visualization.model", mock_model), \
         patch("ml_service.api.visualization.plot_antenna_coverage", return_value=str(img_path)):
...
def test_trajectory_endpoint(client, tmp_path):
    ...
    with patch("ml_service.api.visualization.plot_movement_trajectory", return_value=str(img_path)):
