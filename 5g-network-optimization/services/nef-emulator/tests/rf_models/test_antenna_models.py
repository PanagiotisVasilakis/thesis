...
def test_picocell_fspl_matches_reference(ue_position):
    ...
    assert abs(
        pl - expected) < 0.5, f"Pico FSPL {pl:.2f} vs expected {expected:.2f}"
...
def test_sinr_under_heavy_interference(ue_position):
    ...
    assert - \
        50 < sinr < 50, f"SINR {sinr:.1f} dB outside realistic range (–50 to 50)"
...
def test_deterministic_outputs_without_shadowing(ue_position):
    ...
    assert pl_expected == ant.path_loss_db(
        ue_position, include_shadowing=False)
